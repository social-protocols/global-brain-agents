...
function to_score_data(r::SQLite.Row)::GlobalBrain.ScoreData
    return GlobalBrain.ScoreData(
        r[:tagId],
        ((r[:parentId]==0) ? nothing : r[:parentId]),
        r[:postId],
        r[:parentId]==0 ? nothing : GlobalBrain.NoteEffect(
            (r[:parentId]==0 ? nothing : r[:parentId]),
            r[:postId],
        ...
        r[:selfP],
        GlobalBrain.BernoulliTally(r[:count], r[:total]),
        r[:topNoteId]==0 ? nothing : GlobalBrain.NoteEffect(
            r[:postId],
...
function get_tallies(
    db::SQLite.DB,
    tag_id::Union{Int,Nothing} = nothing,
    post_id::Union{Int,Nothing} = nothing,
...
"""
function insert_score_data(db::SQLite.DB, score_data::GlobalBrain.ScoreData)
    sql_query = """
